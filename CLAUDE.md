...
| 摩尔斯电码 | 选择 2 → 输入文本（如 `SOS`）→ 输入 `E0` 退出 | DB0 引脚 LED 按摩尔斯时序闪烁（见下方详细说明） |
...
- 字母 A-Z（大小写均可，内部统一处理）
- 数字 0-9
- 其他字符（空格、标点等）会被静默跳过，不发送任何信号

#### 时序规则（`morse_Project.c` 中的实现）

| 信号元素 | 引脚状态 | 持续时间 | 代码 |
|---------|---------|---------|------|
| 点 (`.`) | DB0 = HIGH (`0x01`) | 100ms | `usleep(100000)` |
| 划 (`-`) | DB0 = HIGH (`0x01`) | 300ms | `usleep(300000)` |
| 点/划之间的间隔 | DB0 = LOW (`0x00`) | 100ms（1个时间单位） | `usleep(100000)` |
| 字母之间的间隔 | DB0 = LOW (`0x00`) | 额外 200ms（加上前面的100ms共300ms，即3个时间单位） | `usleep(200000)` |

#### 操作流程

1. 在主菜单选择 `2`（Send Morse Code）
2. 提示 `Enter your message (type 'E0' to finish):`
3. 输入文本后回车，程序逐字符转换并发送
4. 可以连续输入多行文本
5. 输入 `E0` 回车退出摩尔斯模式（`E0` 也可出现在行尾，如 `HELLOE0`，此时 `HELLO` 会被发送，遇到 `E0` 停止）

#### 终端输出示例

输入 `SOS` 后终端会打印每个字符对应的摩尔斯码：
```
Morse code for 'S': ...
Morse code for 'O': ---
Morse code for 'S': ...
...
#### 用 `SOS` 验证时序的预期 LED 行为

```
S: 亮100ms 灭100ms 亮100ms 灭100ms 亮100ms 灭100ms [字母间隔 灭200ms]
O: 亮300ms 灭100ms 亮300ms 灭100ms 亮300ms 灭100ms [字母间隔 灭200ms]
S: 亮100ms 灭100ms 亮100ms 灭100ms 亮100ms 灭100ms [字母间隔 灭200ms]
```
总用时约: S(600ms) + 间隔(200ms) + O(1200ms) + 间隔(200ms) + S(600ms) + 间隔(200ms) = ~3000ms
